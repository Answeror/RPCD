...
{
    QImage binary(source.size().width, source.size().height, QImage::Format_Mono);
    for (auto i = source.begin(); i != source.end(); ++i)
    {
        binary.setPixel(QPoint(i.pos().x, i.pos().y), 1);
    }

  //QImage detection = binary;
  ...
  if(max_r == 0)
  {
    max_r = MIN(binary.width(), binary.height()) / 2;
  }
  ...
  for(unsigned int i = min_r; i < max_r; i++)
  {
    ...
    /* loop through all the Hough-space images, searching for bright spots, which
    indicate the center of a circle, then draw circles in image-space */
    //static double dpi = std::acos(-1.0);
    int threshold = static_cast<int>(std::ceil(8 * i * cover_radio));
    int off = (std::max)(int(i), kernel_radius);

    for(unsigned int x = off; x + off < hough.size(); x++)
    {
      for(unsigned int y = off; y + off < hough[x].size(); y++)
      {
        if(hough[x][y] < threshold)
        {
          //draw_circle(detection, QPoint(x, y), i, Qt::yellow);
        ...
        auto val = hough[x][y];
          ...
          if (ok)
          {
              result.push_back(cv::Vec4f(x, y, i, val));
          }
      }
    }
  }
